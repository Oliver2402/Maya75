...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case MINIMIZE:
        if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_LEFT_GUI) "d" SS_UP(X_LEFT_GUI));
        } else {
        }
        break;
    case LOCK:
        if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_LEFT_GUI) "l" SS_UP(X_LEFT_GUI));
        } else {
        }
        break;
    case LOADRIVALS:
        if (record->event.pressed) {
            SEND_STRING(SS_TAP(X_LGUI) SS_DELAY(100) "Marvel Rivals" SS_DELAY(200) SS_TAP(X_ENT) SS_DELAY(2500) SS_TAP(X_LEFT) SS_TAP(X_ENT));
        } else {
        }
        break;
    case HEART:
        if (record->event.pressed) {
            SEND_STRING(SS_DOWN(X_LEFT_ALT) SS_TAP(X_KP_3) SS_UP(X_LEFT_ALT));
        } else {
        }
        break;
    }
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [0] = LAYOUT(
        KC_ESCAPE, KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8, KC_F9, KC_F10, KC_F11, KC_F12, KC_DELETE, KC_AUDIO_MUTE,
        ...
        KC_CAPS_LOCK, KC_A, KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K, KC_L, KC_SEMICOLON, KC_QUOTE, KC_ENTER, KC_PAGE_DOWN, KC_MEDIA_PREV_TRACK,
        KC_LEFT_SHIFT, KC_Z, KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMMA, KC_DOT, KC_SLASH, KC_RIGHT_SHIFT, KC_UP, KC_END, KC_MEDIA_PLAY_PAUSE,
        KC_LEFT_CTRL, KC_LEFT_GUI, KC_LEFT_ALT, KC_SPACE, KC_RIGHT_ALT, KC_APPLICATION, KC_RIGHT_CTRL, KC_LEFT, KC_DOWN, KC_RIGHT, KC_MEDIA_NEXT_TRACK, HEART, MINIMIZE, LOADRIVALS
    )
};
